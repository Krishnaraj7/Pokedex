...
const DetailsPage = () => {
  ...
  return (
    <>
      ...
      <h1 className=' mt-6 ml-6 text-2xl font-semibold capitalize'> {pokemonDetails?.name}</h1>
       </div>
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row gap-8 shadow-xl shadow-gray-700 bg-gray-200 rounded-xl mt-4 py-9 px-5 max-w-[700px] mx-auto">
            ...
            <div className="text-slate-900 font-bold flex-grow">
              ...
              <p className="mb-2">Types: {pokemonDetails.types.map((type) => type.type.name).join(', ')}</p>
              <p className="mb-2">Abilities: {pokemonDetails.abilities.map((ability) => ability.ability.name).join(', ')}</p>
